...
import logoIso from "../../assets/imgs/certificado-iso.png";
import { Sidebar, ToggleDarkMode } from "./partials";
const links = [
  {
    to: "/cursos",
    nombre: "Cursos",
...
const NavItem = ({ to, nombre, href, className }) => {
  return (
    <li className={`text-gray-800 text-sm font-semibold hover:text-purple-600 px-3 py-2 ${className}`}>
      {href === true ? (
  ...
  return (
    <>
      <Link className="min-w-max h-12" to="/">
        <img className="min-w-max h-12" src={logo} alt="Logo Desarrollo Global" />
      </Link>
    </>
  );
};
...
const IconoCarrito = () => {
  return (
    <button type="button" className="ml-2 sm:ml-5">
        ...
        <div className="container mx-auto px-4">
          <nav className="flex items-center justify-between py-4">
            ...
            <div className="hidden sm:flex sm:items-center">
              ...
              <a
                href="/"
                className="text-white bg-primary text-sm font-semibold border px-4 py-2 rounded-lg hover:text-purple-600 hover:border-purple-600"
              >
                Inicia sesión
              </a>
            </div>
          ...
          {/* <div className={`${navShow && "hidden"} block xl:hidden  bg-white border-t-2 py-2`}>
